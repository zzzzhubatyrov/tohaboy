<template>
  ...
  <div class="category-container">
    <div class="header">
      <h1 class="title">Справочник категорий</h1>
    </div>
      ...
      <table v-else class="category-table">
        ...
        <tbody>
          <tr v-for="category in categories" :key="category.id" class="table-row">
            ...
            <td>
              <div class="actions">
                <button @click="editCategory(category)" class="btn-icon" title="Редактировать">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                  </svg>
                </button>
                <button @click="deleteCategory(category.id)" class="btn-icon" title="Удалить">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path d="M3 6h18"/>
    ...
    <div v-if="showModal" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h2>{{ modalMode === 'create' ? 'Добавить категорию' : 'Редактировать категорию' }}</h2>
...
export default {
  ...
  methods: {
    ...
    async saveCategory() {
      try {
        const service = this.modalMode === 'create' ? CreateCategory : UpdateCategory
        const response = await service(this.currentCategory)
